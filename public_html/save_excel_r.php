...
// Define the column names - AGREGADA COLUMNA ETA
$columnNames = array('Id', 'Image', 'Customer Alias', 'Description', 'Supplier', 'Customer', 'TP','UXB', 'QTY', 'CTN', 'PRICE', 'TOTAL AMMOUNT', 'CBM','GW','NW','ETA'); // Replace with your actual column names
 // Replace with your actual column names

// Define the desired column order - ETA ANTES DE IMAGEN
$columnOrder = array(0,4,5,6,2,3,7,8,9,10,11,12,13,14,15,1); // Orden: Id, Supplier, Customer, TP, Customer Alias, Description, UXB, QTY, CTN, PRICE, TOTAL, CBM, GW, NW, ETA, Image
...
foreach ($data as $row) {
    ...
    // Limpiar la columna de imagen del array antes de insertar
    $cleanRow = $reorderedRow;
    $imageIndex = array_search(1, $columnOrder);
    if ($imageIndex !== false) {
        $cleanRow[$imageIndex] = ''; // Limpiar el HTML de la imagen
    }
    
    // Set the rest of the row values (sin HTML de imagen)
    $objPHPExcel->getActiveSheet()->fromArray(array($cleanRow), NULL, 'A' . $rowIndex);
    $rowIndex++;
}

// Configurar columna P para la imagen
$objPHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(15);
$objPHPExcel->getActiveSheet()->setCellValue('P1','Image');

// Save the file to the specified folder
$filename = 'Outstanding.xls';
